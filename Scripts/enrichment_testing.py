...
def enrichment(gene_list, ontology, alpha, all_genes):
    ...
    sig_tup = [(p_tups[i][0], p_tups[i][1] < bonf_alpha)
               for i in range(len(p_tups))]

    return [sig_tup, signif, pvals,
            'Number pathways significant: {}'.format(sum(signif))]
...
def gea_performance(iterations, exp_type, num_paths, percent_path,
                    percent_addit, all_genes_filename, path_genes_filename,
    ...
    :param weights: Weights can either be IMP weights using WEIGHTS w/o quotes
    orno weights using "NULL". Note n can't be too small for the community
    detection methods because insufficent genes to create network. Defaults to
    None if control.
    ...
    results_columns = ['iter_num', 'method', 'num_paths', 'percent_path',
                       'percent_addit', 'true_positive', 'false_positive',
                       'true_negative', 'false_negative']

    summary_results_df = pd.DataFrame(columns=results_columns,
                                      index=range(iterations))

    for iteration in range(iterations):

        selected_path_ids, gene_list = m_gene_list(num_paths, percent_path,
                                                   percent_addit, ALL_GENES,
                                                   PATH_GENES,)
        ...
        if exp_type == 'ctr':
            ...
            for com in cd_com_lst:
                ...
                if ctr_method == 'ctr_m':
                    ...
                    # list of top m paths
                    top_m_paths = [relevant_results[i] for i in
                                   range(len(relevant_results))]

                    # signif paths in the top m paths
                    top_signif_paths = set([top_m_paths[i][0] for i in
                                            range(len(relevant_results))
                                            if top_m_paths[i][1]])

                elif ctr_method == 'ctr_all':
                    top_signif_paths = set([results[0][i][0] for i in
                                            range(len(results[0])) if
                                            results[0][i][1]])
                # paths that are not in the top m and signif
                non_top_paths = set(range(len(PATH_GENES)))
                non_top_paths = non_top_paths.difference(top_signif_paths)
            ...
            for com in cd_com_lst:
                results = enrichment(com, PATH_GENES, alpha, ALL_GENES)
                top_path = results[0][0][0]  # integer id of top path

                if results[0][0][1]:  # is the top path significant? (T/F)
                    top_signif_paths.add(top_path)

                ns_paths = set.difference(set(range(len(PATH_GENES))),
                                          set([top_path]))
