...
#
# Notes: First all genes that have a one-one mapping are immediately mapped to
# their entrezid using the "SYMBOL" option in the select command. Of those
# genes that have a one-many mapping, the "ALIAS" option is used to coerce a
# one-one mapping. 
...
# perform second stage mapping of the genes that did not initially map with
# "SYMBOL" option
mappingNA <- select(org.Hs.eg.db, genesNA, c("ENTREZID", "GENENAME"), "ALIAS")

# creates an association between the gene name and entrezid for those genes
# that did not map using "SYMBOL" option
alias_lst <- mappingNA$ENTREZID
names(alias_lst) <- mappingNA$ALIAS

# iterates through the SYMBOL feature of the mapping object and for those genes
# that did not map through first pass, it replaces the "NA" in enterezid with
# the mapped alias enterezid 
