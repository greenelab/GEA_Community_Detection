...
def data_generation(iterations, num_paths_min, num_paths_max, percent_min,
    ...
    '''
    Description
    Calls gsea_performance() over desired number of iterations for each
    parameterlevel combination of num_paths, percent, and percent_addit to
    create data for enrichment simulation
    ...
    or no weights using "NULL". Note n can't be too small for the community
    ...
    methods = ['ctr_all', 'ctr_m', 'fastgreedy', 'walktrap', 'infomap',
               'multilevel']
        ...
        for num_paths in range(num_paths_min, num_paths_max):
            for percent_path in np.linspace(percent_min, percent_max, 5):
                for percent_addit in np.linspace(addit_min, addit_max, 5):
                    res = gea_performance(iterations, exp_type, num_paths,
                                          percent_path, percent_addit,
                                          all_genes_filename,
                                          path_genes_filename,
                                          ctr_method=ctr_method,
                                          com_method=com_method,
                                          weights=None, min_com_size=None,
                                          alpha=.05)
...
if __name__ == '__main__':
    ...
    data_generation(int(iterations), int(num_paths_min), int(num_paths_max),
                    ...
                    str(all_genes_filename), str(path_genes_filename))
