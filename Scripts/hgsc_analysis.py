...
def convert_string(gene_list):
    ...
    Output
    :A list of genes in string form instead of integer. This is for
    compatibility with PATH_GENES and ALL_GENES
    '''
...
def cd_gea_pathways(all_genes_lst, all_names_lst, com_method, alpha=.05,
    ...
    :alpha: preset to .05
    :min_com_size: minimum size of community for it to be considered for
    further analysis
    :weights: weights used in IMP network, default is None

    Output
    :dataframe with pathway_name, pval, cluster id and community id for each
    significant pathway
    '''
    ...
    for i, lst in enumerate(all_genes_lst):
        ...
        for c, com in enumerate(cd_com_lst):
            ...
            if len(community_melt_df) > 0:
                ...
                # must do zero-index to get the cluster string object
            ...
            cluster_df = cluster_df.append(community_melt_df,
                                           ignore_index=True)

        cluster_df2 = cluster_df.rename(columns={'variable': 'pathway_name',
                                                 'value': 'pval'})
...
def run_all_cd(master_genes_lst, master_namelst):
    ...
    Output
    :dataframe of pathway_name, pval, cluster, community for all community
    detection methods and k values

    '''

    methods = ['fastgreedy', 'walktrap', 'infomap', 'multilevel']
    cols = ['pathway_name', 'pval', 'cluster', 'community', 'method',
            'num_pos_genes_pathway', 'num_neg_genes_pathway',
            'pathway_com_overlap_pos', 'pathway_com_overlap_neg']
    ...
    master_file = os.path.join('Data', 'master_cd_runs.csv')
...
path_names_file = os.path.join('Data', 'PID_path_names.pkl')
PATH_NAMES = pickle.load(open(path_names_file, 'r'))

imp_file = os.path.join('Data', 'IMP_genes.pkl')
IMP_GENES = pickle.load(open(imp_file, 'r'))

hgsc_file = os.path.join('Data', 'entrezid_hgsc.txt')
...
master_namelst = [['k4c1', 'k4c2', 'k4c3', 'k4c4'], ['k3c1', 'k3c2', 'k3c3'],
                  ['k2c1', 'k2c2']]
...
if __name__ == '__main__':
    run_all_cd(master_genes_lst=master_genes_lst,
               master_namelst=master_namelst)
